...
- [x] **P1 (Guest Lockout, Read-Only Enforcements & Stage Clean-Up):** Removed Settings for guests (hidden in LeftRail, guarded in App.tsx and SetupScreen.tsx). Locked FX racks and Mixer Config from being editable in guest mode with read-only banners. Configured Scenes to be selectable/recallable for preview inspection while completely hiding store, duplicate, overwrite, edit, and delete mutations. Removed Stage Item Palette drawer from PhysicalCanvas for an unobstructed stage view.
